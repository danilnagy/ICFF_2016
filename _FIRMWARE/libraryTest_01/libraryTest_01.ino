...
long perlinXInc    = 10000L;
long perlinYInc    = 10000L;

long adjustmentFactor = 100L;

//unsigned long perlinTimePosition = 0;

float targetValue = 2500; // variable to control overall light level
...
class Light {
  ...
  public:
    ...
    Light(int _id, float _x, float _y)
    {
      ...
      y = _y;
      id = _id;
      level = int(renderNoise(x * perlinXInc, y * perlinYInc, 0 * perlinTimeInc));
      adjustment = 0;
    };
...
//        for (int i = 0; i < 3; i++) {
////          println(sensors[i].getRange(x, y));
//          adjustment += sensors[0].getRange(x, y);
//        }
      }
...
//      println(adjustment);
      ...
      level = int(renderNoise(x * perlinXInc, y * perlinYInc, _c * perlinTimeInc)) + adjustmentFactor * adjustment;

    };

    String plot() {
      //      String stringOne = "light: ";
      //      String stringTwo = " value: ";
      //      String stringThree = stringOne + id + stringTwo + level;
